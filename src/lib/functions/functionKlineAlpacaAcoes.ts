...
export async function functionKlineAlpaca({
	...
	if (periodo === '1Day') {
		...
		url = `https://data.alpaca.markets/v2/stocks/bars?symbols=${encodeURIComponent(moeda)}&timeframe=${periodo}&start=${encodeURIComponent(startISO)}&sort=asc`;
	} else if (periodo === '1Min') {
		const now = new Date();
		now.setMinutes(now.getMinutes() - quantidade);
		const timestamp = now.toISOString().split('.')[0] + 'Z'; // Formato correto
		url = `https://data.alpaca.markets/v2/stocks/bars?symbols=${encodeURIComponent(moeda)}&timeframe=${periodo}&start=${timestamp}`;
